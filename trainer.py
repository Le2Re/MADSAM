...
def trainer_khanhha(args, model, snapshot_path, multimask_output, low_res): 
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if args.use_amp:
                with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=args.use_amp):
                    outputs = model(image_batch, multimask_output, args.img_size, (args.img_size,args.img_size))
                    loss, loss_ce, loss_dice = calc_loss(outputs, label_batch, ce_loss, dice_loss, args.dice_param)
                    MSE_loss = mse_loss(outputs['feature_proto'],outputs['decoder_out'])
                    loss = 0.8*loss + 0.2*MSE_loss
            ...
            logging.info('iteration %d : loss : %f, loss_ce: %f, loss_dice: %f, MSE_loss: %f,  lr: %f' % (iter_num, loss.item(), loss_ce.item(), loss_dice.item(),MSE_loss, lr_))
        ...
        if epoch_num  % save_interval == 0 and epoch_num > 95:
